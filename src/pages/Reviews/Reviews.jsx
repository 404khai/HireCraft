    ...
    const fetchReviews = async () => {
      ...
      try {
        ...
        if (user.role === 'PROVIDER' || user.roles?.includes('PROVIDER')) {
          ...
          role = 'provider';
        } else if (user.role === 'CLIENT' || user.roles?.includes('CLIENT')) {
    ...
    if (!searchTerm.trim()) {
      ...
      const filtered = reviews.filter(review => {
        ...
        return (
          ...
          (review.clientFullName && review.clientFullName.toLowerCase().includes(searchLower)) ||
          // Search in provider name (for client reviews) - adjust field name based on your API response
        ...
        <div className="dashboard">
          ...
          <div className="reviewsBox">
            ...
            {filteredReviews.length > 0 ? (
              filteredReviews.map((review, index) => (
                <div className="reviewBoxEach" key={review.id || index}> {/* Use index as fallback key */}
                  <ReviewCard review={review} userRole={userRole} />
                </div>
              ))
            ) : (
              <p>
